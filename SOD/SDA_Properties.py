...
##            " ORDER BY areasymbol, musym, muname, mu.mukey"
        if aggMethod == "Weighted Average":
            ...
            " comppct_r,"\
            " SUM (CASE WHEN hzdepb_r &gt; 30  THEN 30 ELSE hzdepb_r END - CASE WHEN hzdept_r &lt;8 THEN 8 ELSE hzdept_r END) over(partition by c.cokey) AS sum_thickness," + aProp + " INTO #main"\
            " FROM legend  AS l"\
            " INNER JOIN  mapunit AS mu ON mu.lkey = l.lkey AND l.areasymbol LIKE '" + areaSym + "'"\
            " INNER JOIN  component AS c ON c.mukey = mu.mukey"\
            " INNER JOIN chorizon AS ch ON ch.cokey=c.cokey AND hzname NOT LIKE '%O%'"\
            " AND hzdepb_r &gt;8 AND hzdept_r &lt;30"\
            " INNER JOIN chtexturegrp AS cht ON ch.chkey=cht.chkey  WHERE cht.rvindicator = 'yes' AND  ch.hzdept_r IS NOT NULL  AND"\
            " texture NOT LIKE '%PM%' and texture NOT LIKE '%DOM' and texture NOT LIKE '%MPT%' and texture NOT LIKE '%MUCK' and texture NOT LIKE '%PEAT%'"\
...
##            interp = interp.replace("{:}", ";")
        ...
        if len(compDict) > 0:
            ...
            template_table = srcDir + os.sep + 'templates.gdb' + os.sep + 'prop_template_table'
